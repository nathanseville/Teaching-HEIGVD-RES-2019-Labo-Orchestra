...
| | *The Musicians every one second* |
|Question | Who is going to **listen for UDP datagrams** and what should happen when a datagram is received? |
| | *The Auditor should add the muscian to the orchestra list* |
|Question | What **payload** should we put in the UDP datagrams? |
| | *Uuid, creation timestamp, timestamp, instrument, sound* |
|Question | What **data structures** do we need in the UDP sender and receiver? When will we update these data structures? When will we query these data structures? |
| | *A Json object, every time a datagramm is sent, when the autidor receives the datagramm* |
...
| | *It is a package manager for node* |
|Question | What is the `npm install` command and what is the purpose of the `--save` flag?  |
| | *Add the dependices in the package.json file* |
|Question | How can we use the `https://www.npmjs.com/` web site?  |
| | *With pleasure* |
|Question | In JavaScript, how can we **generate a UUID** compliant with RFC4122? |
| | *With an npm package implementing the RFC4122 uuid generation* |
...
| | *Just run the container, entrypoint set the first cmd executed when a container runs* |
|Question | How do we get the list of all **running containers**?  |
| | *docker ps* |
|Question | How do we **stop/kill** one running container?  |
| | *docker stop id* |
|Question | How can we check that our running containers are effectively sending UDP datagrams?  |
| | *Log in when receipt in Auditor, Wireshark* |
...
| | *With a dgram socket register to multicast group with multicast address and call function on message receipt* |
